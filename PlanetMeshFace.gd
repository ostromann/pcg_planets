...
func regenerate_mesh(planet_data : PlanetData):
	...
	for y in range(resolution):
		for x in range(resolution):
			...
			var point_on_unit_cube : Vector3 = normal + (percent.x-0.5) * 2.0 * axisA + (percent.y-0.5) * 2.0 * axisB
			var point_on_unit_sphere := point_on_unit_cube.normalized() * planet_data.radius
#			var biome_percent := planet_data.biome_percent_from_point(point_on_unit_sphere)
...
#			uv_array[i] = Vector2(0.0, biome_percent)
			...
			if l < planet_data.min_elevation:
				planet_data.min_elevation = l
			if l > planet_data.max_elevation:
				planet_data.max_elevation = l
